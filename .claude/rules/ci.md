...
## CI structure
Jobs: Go Tests · Python (runner/query) · TypeScript (SDK+portal) · Lint (go vet + golangci + ruff + buf) · Docker Build · Integration. `npm ci` is used (lock must match package.json). Plugin-test matrix lives under `plugins/` (not `examples/`).
