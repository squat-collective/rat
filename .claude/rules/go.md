...
**Toolchain:** Go 1.24 · chi (router) · ConnectRPC + `google.golang.org/protobuf` · `robfig/cron/v3` · MinIO Go SDK · `pgx` + `sqlc` · `slog` · stdlib `testing` + `testify`.
...
- `make lint-go-strict` runs **golangci-lint v1.64.8** (pinned in `GO_LINT_IMAGE` and `ci.yml`). `make lint` runs `go vet`.
