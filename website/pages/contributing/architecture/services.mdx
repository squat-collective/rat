...
RAT runs as 7 long-lived services plus 2 init jobs (`minio-init` and `nessie-init`). This page provides a deep dive into each service: what it does, how it works internally, and how it connects to the rest of the platform.
...
| **Ports** | `8080` (public REST), `8090` (internal HTTP, not published) |
| **Memory limit** | 512 MB |
| **CPU limit** | 1.0 |
...
- **REST API** --- endpoints covering pipelines, runs, schedules, namespaces, storage, quality tests, metadata, query proxy, landing zones, triggers, versions, plugin management, and platform settings
- **gRPC client** --- dispatches pipeline execution to runner and queries to ratq via ConnectRPC
- **Authentication** --- pass-through (Noop) or static API key by default; an optional auth plugin adds JWT/OIDC
...
- **Plugin host** --- loads, health-checks, and reverse-proxies optional plugins via gRPC + the internal listener
...
<Callout type="info">
`ratd` serves two HTTP listeners: the public API on `8080` and an unauthenticated **internal** listener on `8090` for service-to-service callbacks (run status from the runner) and plugin phone-home (ADR-019). There is no gRPC listener on `8081`.
</Callout>
...
9. Auth                    → Static API key or Noop (an optional auth plugin adds JWT)
...
The binary includes a built-in healthcheck subcommand that hits `http://localhost:8080/health` internally. `/health` (and `/health/live`) is a liveness probe returning build metadata; dependency-aware checks live at `/health/ready`:

```json filename="GET /health/ready"
{
  "status": "ready",
  "checks": {
    "postgres": { "status": "ok" },
...
The readiness keys are `postgres`, `s3`, `runner`, and `query`; the overall status is `ready` or `not_ready`.
...
| `ListPlugins` | List runner plugins (Python entry points) discovered in the runner container. |
...
When a pipeline run completes (success or failure), the runner **pushes** the terminal status back to ratd via an HTTP POST to `RATD_CALLBACK_URL` — which points at ratd's **internal** listener (`http://ratd:8090`), not the public `8080`. This eliminates continuous polling. ratd falls back to polling GetRunStatus every 60 seconds as a safety net.
...
- **Query limits** --- 100 KB max query size; per-query timeout via `QUERY_TIMEOUT_SECS` (default 60s)
...
- **Data explorer** --- browse Iceberg tables, inspect schemas, and preview data
- **Run monitoring** --- real-time log streaming, run history, phase profiling
- **Landing zones** --- file upload, preview, and management
- **Plugin UI host** --- mounts optional plugin surfaces (e.g. lineage DAG) under `/x/...`

### Routes

The portal's App Router routes, organized by feature:

| Route | Description |
|-------|-------------|
| `/` | Dashboard --- overview of recent runs, pipeline counts |
| `/pipelines` | Pipeline browser --- list, filter, search |
| `/pipelines/[ns]/[layer]/[name]` | Pipeline detail --- editor, config, runs, quality |
| `/query` | Query console --- interactive SQL editor |
| `/explorer` | Data explorer --- browse Iceberg tables and data |
| `/explorer/[ns]/[layer]/[name]` | Table detail --- schema, preview, metadata |
| `/runs` | Run history --- all runs across all pipelines |
| `/runs/[id]` | Run detail --- logs, phase timing, error details |
| `/landing` | Landing zone browser |
| `/login` | Login page (used when an auth plugin is active) |
| `/admin/users` | User administration (when an auth/identity plugin is active) |
| `/settings` | Platform settings |
| `/settings/permissions` | Permissions / ACL management (permissions plugin) |
| `/settings/plugins` | Installed plugin management |
| `/settings/retention` | Retention policy configuration |
| `/x/[...path]` | Catch-all route host for plugin-provided UI (e.g. `/x/lineage`) |

<Callout type="info">
Surfaces such as lineage, schedules, quality dashboards, and namespace management that were once standalone routes are now provided either inline (in pipeline/table detail views) or by optional plugins mounted under `/x/...`.
</Callout>
...
| **Port** | host `5433` → container `5432` (localhost only) |
| **Memory limit** | 1 GB |
| **CPU limit** | 1.0 |
| **Networks** | backend |
...
- **19 tables** of platform state (see [Database Schema](/contributing/architecture/database-schema))
...
| **Ports** | host `9002` → 9000 (S3 API, localhost), host `9003` → 9001 (Console, localhost) |
| **Memory limit** | 1 GB |
| **CPU limit** | 1.0 |
| **Networks** | backend |
...
A one-shot init container that runs after Nessie is healthy. A fresh Nessie 0.99 starts `main` at genesis with zero commits, which makes the runner's branch-and-merge flow fail with `404 "no common ancestor"`. `nessie-init` seeds a single bootstrap commit on `main` to fix this for all future runs. It is idempotent (skips the commit if `main` already has history). The long-running services depend on it via `condition: service_completed_successfully`.
...
See [Nessie Branching](/contributing/architecture/nessie-branching) for the full branch lifecycle.
...
4. **nessie-init** (depends on nessie healthy; seeds genesis commit, then exits)
5. **runner** + **ratq** (depend on minio + nessie healthy + nessie-init completed)
6. **ratd** (depends on postgres + minio + nessie healthy + nessie-init completed)
7. **portal** (depends on ratd healthy)
