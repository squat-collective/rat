...
sequenceDiagram
    ...
    Runner->>ratd: POST /api/v1/internal/runs/{id}/status (to :8090)
...
The runner performs branch creation, config loading, SQL execution, Iceberg writes, and quality testing. See [Pipeline Execution](/contributing/architecture/pipeline-execution) for the full breakdown.
...
The user types SQL in the CodeMirror editor and clicks "Run". The portal sends `POST /api/v1/query` with the raw SQL text.

### ratd validates and proxies

ratd performs basic validation (query size under 100 KB, timeout set to 60 seconds) and forwards the query to ratq via the `ExecuteQuery` gRPC call. ratd acts as a proxy --- it does not execute SQL itself.
...
Queries have a **60-second timeout** and a **100 KB size limit**. If your query hits the timeout, consider optimizing it with filters, limits, or by creating a Gold-layer pipeline for pre-aggregation.
</Callout>
...
sequenceDiagram
    ...
    Portal->>ratd: POST /api/v1/files/upload
...
The user drags a file into a landing zone in the portal. The portal sends a multipart upload to `POST /api/v1/files/upload` (landing-zone uploads use the landing-zone file endpoints). ratd validates the file (size limits, allowed content types, no path traversal) and writes it to MinIO at `s3://rat/{namespace}/landing/{zone_name}/{filename}`.
...
sequenceDiagram
    ...
    Portal->>ratd: GET /api/v1/schema?table=...
...
|-----------|------|----------|
| Pipeline run | Portal → ratd → Runner → Nessie/MinIO → ratd (callback to :8090) | REST + gRPC + S3 + REST |
| Query | Portal → ratd (`POST /api/v1/query`) → ratq → MinIO (read) | REST + gRPC + S3 |
| File upload | Portal → ratd (`POST /api/v1/files/upload`) → MinIO | REST + S3 |
