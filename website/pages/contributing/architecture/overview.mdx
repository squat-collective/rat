...
RAT is a self-hostable data platform that runs as **7 services plus 2 init jobs** orchestrated by Docker Compose. It follows a strict separation of concerns: a Go API server handles orchestration and state, Python services handle data processing, and a Next.js frontend provides the user interface.
...
1. **Single `docker compose up`** --- The entire platform starts with one command. No external dependencies, no cloud accounts, no license keys. RAT is fully free and open-source.
...
4. **Plugin extensibility** --- The core ships with pass-through (no-op) defaults for capabilities like auth and sharing. Free optional plugins slot in to add multi-user, sharing, cloud, and a container executor without changing the core platform.
...
The following diagram shows all containers (7 services + 2 init jobs), their communication protocols, network zones, and exposed ports.

<Mermaid chart={`
block-beta
    columns 4

    block:frontend["🌐 Frontend Network (rat_frontend)"]:4
        portal["Portal\n(Next.js)\n:3000"]
        ratd["ratd\n(Go)\n:8080 REST\n:8090 internal"]
    end
    ...
    block:backend["🔒 Backend Network (infra_default)"]:4
        ratq["ratq\n(Python)\n:50051 gRPC"]
        runner["runner\n(Python)\n:50052 gRPC"]
        postgres["Postgres\n16.4\n:5432"]
        minio["MinIO\n(S3)\n:9000/:9001"]
...
| **ratd** | Go | API server, scheduler, plugin host, catalog ops | `8080` (public REST), `8090` (internal HTTP) | frontend + backend |
...
| **postgres** | PostgreSQL 16.4 | Platform state (19 tables) | host `5433` → 5432 (localhost only) | backend |
| **minio** | MinIO | S3-compatible object storage | host `9002`/`9003` → 9000/9001 | backend |
...
| **nessie-init** | Alpine | One-shot: seeds a genesis commit on `main` | --- (exits after setup) | backend |
...
flowchart LR
    ...
    ratd -->|ConnectRPC gRPC| plugins["Optional plugins"]
...
| runner to ratd | **HTTP callback** | Push-based status reporting. Runner POSTs terminal status to ratd's internal listener (`:8090`) on completion, with 60s poll fallback. |
...
The internal network where all inter-service communication happens. **All containers** are attached to this network, but only `portal` and `ratd` are also on the frontend network. ratd's internal listener (`:8090`) lives on this network and is never published to the host.
...
In production, the localhost-bound debug ports for Postgres (host `:5433`), MinIO (host `:9002`/`:9003`), and Nessie (`:19120`) should be removed or firewalled. They exist for development convenience only.
</Callout>
...
### Nessie starts, then nessie-init runs

**Nessie** depends on Postgres (it persists catalog metadata via JDBC). It starts once Postgres is healthy and exposes the Iceberg REST catalog on port 19120. The one-shot **nessie-init** then seeds a genesis bootstrap commit on `main` so the runner's branch-and-merge flow works, and exits.

### Python services start

**runner** and **ratq** start once MinIO and Nessie are healthy and `nessie-init` has completed. They initialize their DuckDB engines with S3 and Iceberg extensions.

### ratd starts

**ratd** depends on Postgres, MinIO, and Nessie (and `nessie-init` completed). On startup it runs database migrations, initializes the scheduler, starts the reaper daemon, connects to runner and ratq via gRPC, and loads any configured plugins.
...
| nessie-init | 64 MB | 0.25 | 50 | One-shot, exits immediately |
...
See the [Security](/contributing/architecture/security) page for the complete security model.
...
| Platform state | Postgres | 19 relational tables |
| Run logs | Postgres (JSONB column) | Structured log entries |

See [Storage Layout](/contributing/architecture/storage-layout) for the full S3 directory structure and [Database Schema](/contributing/architecture/database-schema) for the Postgres tables.
...
<Cards>
  <Card title="Services Deep Dive" href="/contributing/architecture/services">
    Detailed breakdown of each service's responsibilities and internals
  </Card>
  <Card title="Data Flow" href="/contributing/architecture/data-flow">
    Sequence diagrams showing how data moves through the system
  </Card>
  <Card title="Pipeline Execution" href="/contributing/architecture/pipeline-execution">
    The 5-phase execution model in detail
  </Card>
  <Card title="Security Model" href="/contributing/architecture/security">
