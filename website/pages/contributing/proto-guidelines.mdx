...
├── runner/v1/runner.proto            # Runner service (7 RPCs)
├── query/v1/query.proto              # Query service (4 RPCs)
├── plugin/v1/plugin.proto            # Base PluginService (HealthCheck, Describe)
├── identity/v1/identity.proto        # Identity / auth plugin contract
├── permission/v1/permission.proto    # Access enforcement (ACL) plugin contract
├── sharing/v1/sharing.proto          # Sharing plugin contract
├── cloud/v1/cloud.proto              # Cloud credential vending plugin contract
├── executor/v1/executor.proto        # Container executor plugin contract
└── common/v1/common.proto            # Shared types (Layer, RunStatus, log/status messages)
```

<Callout type="warning">
The old `auth/v1` and `enforcement/v1` protos have been **removed**. Their responsibilities now live in `identity/v1` (authentication) and `permission/v1` (enforcement). The five typed plugin protos (`identity`, `permission`, `sharing`, `cloud`, `executor`) are the live plugin contract — do not delete them.
</Callout>
...
- Service name: `runner`, `query`, `plugin`, `identity`, `permission`, `sharing`, `cloud`, `executor`, `common`
- Version: `v1`, `v2`, etc.
- Full pattern: `ratatouille.{service}.{version}`

<Callout type="info">
Wire-protocol package names are **frozen** as `ratatouille.*` even though the product is now called RAT. Renaming them breaks the wire protocol with deployed plugins.
</Callout>
...
service RunnerService {
  ...
  // Get the current status of a run (pending, running, success, failed).
  rpc GetRunStatus(GetRunStatusRequest) returns (GetRunStatusResponse);

  // Stream real-time log entries for a run. Server-side streaming.
  rpc StreamLogs(StreamLogsRequest) returns (stream LogEntry);

  // Cancel a running pipeline. No-op if already finished.
  rpc CancelRun(CancelRunRequest) returns (CancelRunResponse);

  // Preview a pipeline without writing to the data lake (LIMIT + profiling).
  ...
  // List runner plugins (Python entry points) discovered in the runner container.
...
Every plugin implements this base service:
...
service PluginService {
  ...
  // Describe — returns the plugin's capabilities, routes, event
  // subscriptions, UI descriptors, and config schema.
  rpc Describe(DescribeRequest) returns (DescribeResponse);

  // HandleEvent — delivers a platform event (e.g., run_completed).
  rpc HandleEvent(HandleEventRequest) returns (HandleEventResponse);

  // Authenticate — validate a bearer token (plugins declaring the "auth" capability).
  rpc Authenticate(AuthenticateRequest) returns (AuthenticateResponse);

  // Authorize — check an action on a resource (plugins declaring the "enforcement" capability).
...
<Callout type="info">
The success terminal state is `RUN_STATUS_SUCCESS` (value `3`) — not `COMPLETED`. The corresponding REST/SDK run status string is `success`.
</Callout>
...
plugins:
  ...
  # Python (runner + query) — pinned to v33 for protobuf 6.33.x runtime compatibility
...
<Callout type="info">
The Go managed `go_package_prefix` is `github.com/rat-data/rat/platform/gen`. The Python codegen plugin is pinned to `v33` so generated `*_pb2.py` target the protobuf 6.33.x runtime installable from PyPI.
</Callout>
