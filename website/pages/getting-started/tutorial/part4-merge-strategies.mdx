...
**Time:** ~15 minutes | **Prerequisites:** [Part 3](/getting-started/tutorial/part3-building-layers) completed (Silver pipeline running)
...
| `incremental` | Upsert rows on the `unique_key` (insert new keys, replace existing) | Event logs, time-series, append-heavy data |
...
| `snapshot` | Replace the matching partition with the new data | Audit trails, compliance reporting |
...
- **First run:** `is_incremental()` returns `true` (the strategy is `incremental`), but
  `watermark_value` is empty/`None` because there's no existing table yet. The
  `and watermark_value` guard is falsy, so the `WHERE` clause is skipped and all 25 rows
  are loaded.
- **Subsequent runs:** `is_incremental()` is still `true`, and `watermark_value` now
  contains the maximum `launch_date` from the existing table. Only rows newer than that
  date are processed, and those rows are **upserted** on the `unique_key` (`launch_id`) —
  new keys are inserted, and rows whose key already exists are replaced, rather than
  blindly appended.
...
|--------|---------|-------------|
| `is_incremental()` | `bool` | `True` whenever the merge strategy is `incremental` |
