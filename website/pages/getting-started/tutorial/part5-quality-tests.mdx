...
# Part 5: Quality Tests 🛡️

**Time:** ~10 minutes | **Prerequisites:** [Part 4](/getting-started/tutorial/part4-merge-strategies) completed (incremental pipeline running)
...
In this part, you'll add **quality tests** to your pipelines — SQL checks that run
...
is clean. If it returns rows, those rows are the violations — and RAT logs exactly
...
|----------|-----------|----------|
| `error` | 🚫 **Blocks merge** | The pipeline branch is discarded. Data does NOT land in the table. |
| `warn` | ⚠️ **Allows merge** | Data is merged normally, but the failure is logged and visible in the UI. |

<Callout type="warning">
Use `error` severity for checks that protect data integrity — null primary keys,
referential integrity, schema violations. Use `warn` for checks that flag
anomalies but shouldn't block the pipeline — unexpected enum values, outliers,
...
Navigate to **Pipelines** → click on **mission_log** to open its detail page.
...
| `@remediation` | *(text)* | What to do when this test fails — helps the on-call engineer |
...
<Callout type="info">
The `ref()` function works in quality tests too — it references the same pipeline
...
`'delayed'` or `'anomaly'` appears in the source data, this test will flag it — but
...
|------|----------|---------------|
| `no-null-ids` | `error` | Primary key completeness — blocks merge on failure |
| `valid-outcomes` | `warn` | Enum validation — logs warning on failure |
...
1. **Branch** — create an isolated Iceberg branch for the write
2. **Execute** — run the pipeline SQL and write results to the branch
3. **Test** — run all published quality tests against the branch data
4. **Merge** — if all `error` tests pass, merge the branch into the main table
...
- **no-null-ids**: ✅ Passed (0 rows returned — no null IDs in our data)
- **valid-outcomes**: ✅ Passed (0 rows returned — all outcomes are recognized values)
...
- The branch would be **discarded** — bad data never reaches the table
...
- The run status would still show **Success** — there is no separate "Warning" run status; `warn`-severity failures do not change the run's outcome
...
like a pull request for your data — bad data never touches production.
</Callout>
...
- **One assertion per test** — don't combine "no nulls" and "valid range" in one query.
  Separate tests make failures easier to diagnose.
- **Use `error` sparingly** — reserve it for true data integrity violations. Overusing
  `error` means your pipelines get blocked frequently.
- **Write actionable remediation** — "check the upstream CSV" is more helpful than
  "data is bad". The person reading this at 2 AM will thank you.
- **Tag consistently** — use standard tags like `completeness`, `validity`, `uniqueness`,
...
- ✅ Created an **error-level** quality test (`no-null-ids`) that blocks merges on failure
- ✅ Created a **warn-level** quality test (`valid-outcomes`) that logs but doesn't block
- ✅ Learned the **branch-test-merge** execution flow
- ✅ Explored quality test **annotations** and best practices
- ✅ Understood the **published vs draft** lifecycle
