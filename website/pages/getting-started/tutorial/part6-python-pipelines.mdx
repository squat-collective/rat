...
**Time:** ~10 minutes | **Prerequisites:** [Part 5](/getting-started/tutorial/part5-quality-tests) completed (quality tests running)

So far, every pipeline you have written has been pure SQL. That covers most use cases — but
sometimes you need the flexibility of Python. Multi-step transformations, programmatic query
building, intermediate variables, logging — Python pipelines let you do all of that while
...
| `config` | `PipelineConfig` or `None` | The parsed pipeline config as a **frozen `PipelineConfig` object** (read fields as attributes, e.g. `config.merge_strategy`). It is `None` when there is no config block — it is **not** a dict. |
...
| `globals()` / `locals()` / `vars()` | Scope manipulation |
| `getattr()` / `setattr()` / `delattr()` | Attribute access (reaches `__subclasses__`) |
| `type()` / `isinstance()` / `hasattr()` | Type introspection chains |
| `breakpoint()` | Debugger access |
| `input()` / `help()` | Would hang the runner |

<Callout type="info">
`__import__` is **not** blocked — it is replaced with a *restricted* importer. Normal
`import` statements work; importing a blocked module raises `ImportError`. (PyArrow's
C++ code calls `__import__` internally, so removing it entirely would crash the runner.)
</Callout>

### Allowed Imports

You **can** import these modules — anything not on the blocklist that happens to be
installed:
...
| **Utilities** | `hashlib`, `uuid`, `struct`, `io`, `copy`, `enum`, `warnings`, `contextlib` |
| **Typing** | `dataclasses`, `typing` |

### Blocked Imports

The importer rejects these modules — notably `os`, `sys`, `subprocess`, `socket`, `http`,
`pathlib`, `tempfile`, `glob`, `importlib`, `ctypes`, `multiprocessing`, `threading`,
`concurrent`, `builtins`, `inspect`, `gc`, `types`, `abc`, `pickle`, and `marshal`.

<Callout type="info">
Network libraries like `requests` and `urllib` are **not** on the blocklist — they
simply are not installed in the pipeline environment. If you need to fetch data from an
HTTP endpoint, use DuckDB's built-in HTTP extension via `duckdb_conn`, which can reach
external URLs.
</Callout>
...
This shows vehicles that have experienced at least one failure — useful for identifying
reliability trends.
...
- ✅ Learned the available globals (including the frozen `config` object) and how they work
...
**Next:** In Part 7, you'll set up triggers to automate your entire pipeline chain — cron
