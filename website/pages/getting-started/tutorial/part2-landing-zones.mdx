...
**Prerequisites:** Completed [Part 1](/getting-started/tutorial/part1-first-pipeline)
...
Click **Create**. Both **namespace** and **name** are required — a landing zone is addressed as `{namespace}/{name}`.
...
curl -X POST http://localhost:8080/api/v1/landing-zones/default/launch-data/files \
...
<Tabs.Tab>
```bash filename="Terminal"
curl -X POST http://localhost:8080/api/v1/landing-zones/default/launch-data/samples \
  -F "file=@docs/data/space_launches.csv"
```
</Tabs.Tab>
</Tabs>
...
```bash filename="Terminal"
# Upload data file
curl -X POST http://localhost:8080/api/v1/landing-zones/default/vehicle-catalog/files \
  -F "file=@docs/data/launch_vehicles.csv"

# Upload sample file
curl -X POST http://localhost:8080/api/v1/landing-zones/default/vehicle-catalog/samples \
  -F "file=@docs/data/launch_vehicles.csv"
```
</Tabs.Tab>
</Tabs>
