...
The left sidebar has **7 core navigation items** (an eighth, **Lineage**, appears only when the optional `rat-plugin-lineage` plugin is installed):
...
| **Lineage** | Visual DAG showing data flow between pipelines (optional plugin, route `/x/lineage`) |
...
<Callout type="info">
Preview runs your SQL in the **runner** — it compiles the Jinja and executes the query through DuckDB with a `LIMIT`, using the same path as a real run (not the ratq query service). Nothing is written
