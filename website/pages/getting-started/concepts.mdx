...
<Callout type="info">
Namespaces are purely organizational by default. The optional multi-user/sharing plugins
can add per-namespace access controls and ownership policies on top — RAT itself is 100%
free and open-source, with no editions or tiers.
</Callout>
...
SELECT *
FROM read_csv_auto('{{ landing_zone('order_uploads') }}')
```
...
| `landing_zone('name')` | Resolve a landing zone's file path (wrap in a reader) | `read_csv_auto('{{ landing_zone('csv_uploads') }}')` |
| `this` | Current pipeline's output table | `{{ this }}` (used in quality tests) |
| `is_incremental()` | True when the merge strategy is `incremental` | `{% if is_incremental() %}` |
...
Beyond the merge strategy, a pipeline's `config.yaml` (or inline `-- @key: value`
annotations) accepts a few more options:

| Field | Default | Description |
|---|---|---|
| `materialized` | `table` | `table` writes an Iceberg table; `view` registers a view instead of writing data |
| `merge_strategy` | `full_refresh` | How rows are written (see above) |
| `unique_key` | — | Column(s) used by `incremental`, `delete_insert`, and `scd2` |
| `partition_by` | — | A list of `{column, transform}` entries defining the Iceberg partition spec. `transform` is one of `identity`, `year`, `month`, `day`, `hour` |
| `max_retries` | `0` | If a run fails, the runner automatically retries it up to this many times (`0` disables auto-retry) |
| `retry_delay_seconds` | `30` | Fixed delay between automatic retry attempts |
...
<Callout type="info">
In Python pipelines this configuration is exposed as a read-only `config` global — a frozen
`PipelineConfig` object (or `None` when no config block is present), not a dict.
</Callout>
...
4. A Bronze pipeline reads the files using `read_csv_auto('{{ landing_zone('zone_name') }}')`
...
It is provided by the optional `rat-plugin-lineage` plugin (not part of the core
platform). Once installed, the plugin builds the graph automatically by parsing `ref()`
and `landing_zone()` calls in your pipeline code, adds a **/x/lineage** navigation item to
the portal, and serves the graph at `GET /api/v1/x/lineage/graph`. See the
[lineage guide](/guides/lineage).
...
| **Lineage** | Dependency DAG between pipelines | Optional plugin (`rat-plugin-lineage`), built from ref() calls |
