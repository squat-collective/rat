...
- **Metadata** — Description, tags, and owner (single user by default; multi-user is an optional plugin).
...
Lineage is provided by the optional `rat-plugin-lineage` plugin — it is not part of the
portal core. Once installed, the plugin adds a **Lineage** navigation item (at
**/x/lineage**) that shows a directed acyclic graph (DAG) of how your pipelines and tables
relate to each other. RAT is 100% free and open-source, so this is a free install, not a
paid feature. See the [lineage guide](/guides/lineage).
...
<Callout type="info">
With the plugin installed, the lineage graph is generated automatically from your pipeline
code. Every time you use `ref()` or `landing_zone()` in your SQL, the plugin parses these
references and builds the dependency graph. No manual configuration is needed.
</Callout>
...
- **Docs** — An editable documentation area. Write descriptions for the table
  and individual columns. This documentation is saved through the table-metadata API
  (`PUT /api/v1/tables/{ns}/{layer}/{name}/metadata`) and stored in Postgres.
...
- **Version info** — Shows the current version number and build info. RAT is 100% free and
  open-source — there are no editions or tiers.
- **Retention configuration** — Set how long run records, logs, and data snapshots are
  ...
  - Run record retention (default: 90 days)
  - Log retention (default: 30 days)
  - Iceberg snapshot retention (default: 7 days)
...
<Callout type="info">
Optional plugins can add settings sections for multi-user management, authentication
providers, and sharing permissions. These are free, installable plugins — not a paid tier.
</Callout>
...
|---|---|
| **Home** | Platform overview, stats, recent runs |
| **Pipelines** | Create, edit, manage data pipelines |
| **Runs** | Monitor pipeline executions |
| **Query** | Interactive SQL against your data |
| **Lineage** | DAG visualization of pipeline dependencies (optional `rat-plugin-lineage` plugin) |
| **Explorer** | Browse tables, schemas, and data |
| **Landing** | Upload and manage raw data files |
