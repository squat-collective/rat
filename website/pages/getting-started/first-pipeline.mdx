...
<Callout type="info">
Preview executes the SQL in the **runner** — it compiles the Jinja and runs the query
through DuckDB with a `LIMIT`, using the same compilation path as a real run (not the
ratq query service). It shows you the output shape and data without writing anything to
the Iceberg catalog. Use it to iterate quickly on your SQL.
</Callout>
...
<Cards>
  <Card title="Add a Silver pipeline" href="/guides/sql-pipelines">
