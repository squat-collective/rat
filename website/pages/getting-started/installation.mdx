...
You should see the RAT Portal dashboard. No login is required out of the box — RAT is a single-user platform by default. (Multi-user auth is available as an optional plugin; RAT itself is 100% free and open-source, with no editions or tiers.)
...
`.env` file. See the [Deployment](/self-hosting) section for guidance.
</Callout>
...
See the full [Upgrading guide](/self-hosting/upgrading) for backup procedures, rollback, and migration details.
