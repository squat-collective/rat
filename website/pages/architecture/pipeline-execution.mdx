...
```
Annotations (-- @key: value in SQL)   ← highest priority
            ↓
config.yaml (in pipeline directory)    ← medium priority
            ↓
...
<Callout type="info">
The reaper runs every 60 minutes (configurable) and performs 6 cleanup tasks. See the [Reaper section](/architecture/services#ratd--platform-api-server) for all tasks.
</Callout>
