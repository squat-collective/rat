...
| `@retry_delay_seconds` | integer | `30` | — | Delay between retries |
...
| Missing `@unique_key` for `incremental`/`delete_insert`/`scd2` strategy | Warning (logged) — falls back to `full_refresh`, run continues |
| Missing `@watermark_column` for `incremental` strategy | Warning (logged) — falls back to `full_refresh`, run continues |
| Missing `@partition_column` for `snapshot` strategy | Warning (logged) — falls back to `full_refresh`, run continues |
