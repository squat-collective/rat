...
<Callout type="warning">
Using `{{ this }}` only makes sense in incremental or append-only pipelines where the target table already exists. Guard the `{{ this }}` branch with `{% if is_incremental() and watermark_value %}` so it's skipped on the first run — `is_incremental()` is `true` from the very first run, but `watermark_value` is empty until the table has been materialized.
</Callout>
...
<Callout type="info">
The `watermark_value` is computed by running `SELECT MAX(watermark_column) FROM target_table` before the pipeline SQL is executed. On the first run (when the table does not exist yet), `watermark_value` is empty/`None`, so the `WHERE col > watermark` filter selects every row — `is_incremental()` is still `true`. Guard with `{% if is_incremental() and watermark_value %}` if you want to skip the block entirely on the first run.
</Callout>
...
Returns `true` whenever the pipeline's merge strategy is `incremental` — on every run, including the first. It does **not** check whether the target table exists. What differs on the first run is `watermark_value`, which is empty until the table has been materialized at least once.
...
| Run | `watermark_value` | Strategy | `is_incremental()` | Behavior |
|---|---|---|---|---|
| First | empty/`None` | incremental | `true` | Full load (filter matches all rows) |
| Subsequent | `MAX(watermark)` | incremental | `true` | Incremental (watermark filter applied) |
| Any | — | full_refresh | `false` | Always full load |
