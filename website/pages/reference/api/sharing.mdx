...
<Callout type="info">
Sharing is provided by an optional plugin. When the plugin is not installed, these endpoints return `501 Not Implemented`. Install/enable the sharing plugin (for example via the `infra/docker-compose.plugins.yml` overlay) to use them.
</Callout>
...
| `501` | `UNAVAILABLE` | Sharing plugin not installed |
