...
|---|---|---|
| `entries` | `array` | List of audit log entry objects |
| `entries[].id` | `string` | Entry UUID |
| `entries[].user_id` | `string` | User who performed the action. Defaults to `"anonymous"` when no auth plugin is active |
| `entries[].action` | `string` | HTTP method in lowercase: `post`, `put`, `delete` |
| `entries[].resource` | `string` | URL path of the API endpoint that was called |
| `entries[].detail` | `string` | Additional detail (currently empty, reserved for future use) |
| `entries[].ip` | `string` | Client IP address |
| `entries[].created_at` | `string` | ISO 8601 timestamp of when the action occurred |
| `total` | `integer` | Total number of matching audit entries (for pagination) |
...
|---|---|---|
| `user_id` | Auth context | Extracted from the JWT token when an auth plugin is active; otherwise `"anonymous"` |
