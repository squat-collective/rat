...
| `pipelines[].id` | `string` | Pipeline UUID |
...
| `pipelines[].owner` | `string\|null` | Owner user ID (set when an auth/ownership plugin is active) or `null` |
...
Returns the full details for a single pipeline, including metadata.
...
| `403` | `AUTHORIZATION` | Insufficient permissions (when an ownership/ACL plugin is active) |
| `404` | `NOT_FOUND` | Pipeline not found |
