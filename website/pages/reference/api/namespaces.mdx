...
By default, namespaces function as organizational groups (no access control). When the optional permissions/ACL plugins are installed, namespaces integrate with the ACL system for multi-tenant access control.
...
|---|---|---|---|
| `name` | `string` | Yes | Namespace name (lowercase letters, digits, hyphens, underscores; must start with a letter) |
...
Deleting a namespace does **not** automatically delete the pipelines, tables, landing zones, and other resources within it. You must delete those resources first, or they will become orphaned. With the optional permissions plugin installed, namespace deletion requires `admin` permission on the namespace.
</Callout>
...
| Hyphens allowed | `my-team` | — |
| Underscores allowed | `my_team` | — |
| Must start with a letter | `team1` | `1team` |
| 1-128 characters | `dev` | (empty) |
