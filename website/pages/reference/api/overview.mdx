...
RAT is a single, free, open platform. It supports the following authentication modes:

### No Authentication (default)

By default, RAT requires no authentication. All endpoints are open — suitable for single-user, local, or trusted-network deployments.

### API Key

When the `RAT_API_KEY` environment variable is set, every request must carry the key as a Bearer token in the `Authorization` header:

```bash
curl -H "Authorization: Bearer your-api-key" http://localhost:8080/api/v1/pipelines
```

### Bearer Token via an auth plugin

When an optional authentication plugin is enabled (for example a Keycloak-based OIDC integration), all requests must include a valid JWT bearer token:
...
| `501` | `UNAVAILABLE` | Feature not available (e.g., sharing when the sharing plugin is not installed) |
...
List endpoints support offset/limit pagination using `limit` and `offset` query parameters:
...
Some list endpoints support sorting via a single `sort` query parameter. Prefix the field name with `-` for descending order:

| Parameter | Type | Default | Description |
|---|---|---|---|
| `sort` | `string` | `created_at` | Field to sort by. Ascending by default; prefix with `-` for descending (e.g. `-created_at`) |

### Example

```bash
# Ascending by name
curl "http://localhost:8080/api/v1/pipelines?sort=name"

# Descending by created_at
curl "http://localhost:8080/api/v1/pipelines?sort=-created_at"
```

Available sort fields vary by endpoint — refer to each endpoint's documentation for details. Unknown sort fields are ignored.
...
| Lineage (plugin) | 1 | [Lineage](/reference/api/lineage) |
| Sharing (plugin) | 4 | [Sharing](/reference/api/sharing) |

In addition to the core families above, optional plugins expose further endpoint families under `/api/v1/x/<plugin>/...` (lineage, permissions, identity, cloud credential vending, plugin management, metadata, and the generic plugin proxy). The current-user endpoint `GET /api/v1/me` is available when an auth/identity plugin is active.
