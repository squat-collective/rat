...
<Callout type="info">
Lineage lives in an optional plugin (`rat-plugin-lineage`), not in core `ratd`. When the plugin is installed, `ratd` reverse-proxies it at `/api/v1/x/lineage/graph`. Without the plugin, this route returns `404 Not Found`.
</Callout>
...
|---|---|---|
| `GET` | `/api/v1/x/lineage/graph` | Get the lineage graph |
