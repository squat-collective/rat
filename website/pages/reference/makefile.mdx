...
`make` is the single entry point for every recurrent command in RAT (~46 targets). All targets run inside Docker containers — nothing is installed on the host machine. If you find yourself typing a raw command more than once, it should be a Makefile target.
...
For recurrent workflows, use `make <target>` rather than raw `go test`, `pytest`, `npm run`, or `docker compose` commands — the targets pin tooling versions and match CI. Two pragmatic exceptions: tailing a **single** service's logs (`docker compose -f infra/docker-compose.yml logs -f ratd`) and iterating on a **single** test during development.
</Callout>
...
These targets manage the Docker Compose stack (7 services + 2 init jobs).
...
RAT has ~46 Makefile targets, with testing being one of the largest groups. All tests run inside Docker containers.
...
Exposes port 8080 (public REST) and the internal listener on 8090 on the host. Connects to the `infra_default` Docker network so it can reach Postgres, MinIO, and other services.
...
| `make up` | Start all services (7 services + 2 init jobs) |
