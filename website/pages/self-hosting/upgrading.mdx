...
RAT releases use a `0.x` scheme while the plugin contract is still being stabilized:
...
|---|---|---|
| **0.MINOR** | New features and possible breaking changes (the contract isn't frozen yet) | 0.2.0 → 0.3.0 |
| **0.x.PATCH** | Bug fixes, backward-compatible | 0.2.0 → 0.2.1 |

Releases are tagged in Git with the version number (e.g., `v0.2.0-beta.1`); the git tag is the single source of truth for the version.

<Callout type="info">
"v2" is the **codename** for the current-generation rewrite of RAT — **not** the SemVer major. Released artifacts are versioned as `0.x`. Once the plugin contract is frozen, RAT will move to `1.0`.
</Callout>
...
gh release view v0.3.0 --repo squat-collective/rat
```

### Create a backup

Always backup before upgrading. See the [Backup & Restore](/self-hosting/backup-restore) page for details.
...
```bash filename="Terminal"
curl -fsSL https://raw.githubusercontent.com/squat-collective/rat/main/install/install.sh | bash -s -- --version=0.3.0 --dir=.
```
...
This creates a timestamped backup of both PostgreSQL and MinIO. See the [Backup & Restore](/self-hosting/backup-restore) page for details.

### Pull the latest code

```bash filename="Terminal"
git fetch origin
git checkout v0.3.0   # checkout the target version tag
```
...
2. Checks which migrations have already been applied (tracked by filename)
3. Applies any pending migrations in order (the current set goes up to `018_failed_merges.sql`)
...
```bash filename="Terminal"
git checkout v0.2.0   # your previous working version
```
...
<Callout type="error">
Never run `ratd` version 0.3.0 with a runner at version 0.2.0. Version skew between services can cause gRPC protocol errors, data corruption, or silent behavior changes. Always upgrade all services together.
</Callout>
