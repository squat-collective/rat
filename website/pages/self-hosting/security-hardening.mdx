...
### Plugin Authentication (optional auth plugin)

For multi-user setups, install the optional auth plugin via the `auth` slot. The most common is `auth-keycloak`, which validates JWT tokens against a Keycloak instance. `plugins` is a map keyed by slot name → `{ addr, config }`:

```yaml filename="rat.yaml"
plugins:
  auth:
    addr: auth-keycloak:50060
    config:
...
# Development (CORS is off by default — set this when the portal runs on a different origin)
...
|---|---|---|
| `RATE_LIMIT` | `50` req/s | On/off switch, not a tunable number. Any non-zero value enables a fixed 50 req/s per-IP limiter; `"0"` disables it |

### Enabling / disabling

`RATE_LIMIT` is **not** a tunable threshold — `ratd` only checks whether it is non-zero. Any non-zero value enables the same fixed 50 req/s per-IP limiter; the number itself is ignored.

```bash
# Enable the built-in 50 req/s per-IP limiter (any non-zero value works)
RATE_LIMIT=1
...
postgres:
  ports:
    - "127.0.0.1:5433:5432"  # localhost only (host 5433 → container 5432)
```
