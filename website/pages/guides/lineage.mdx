...
Data lineage shows you how data flows through your pipelines — which tables feed into which pipelines, and which pipelines produce which tables. Lineage is provided by the optional **`rat-plugin-lineage`** plugin; it is not part of the platform core. Once installed, the plugin scans your pipeline SQL for `ref()` and `landing_zone()` calls and renders the result as an interactive DAG (Directed Acyclic Graph).

<Callout type="info">
RAT is 100% free and open-source — there are no editions or paid features. Lineage is an installable plugin rather than a paywalled feature. Once installed it registers a **/x/lineage** navigation item in the portal and serves its API under `/api/v1/x/lineage`. See [writing plugins](/guides/writing-plugins) for how plugins are installed.
</Callout>
...
With the `rat-plugin-lineage` plugin installed, the Lineage page is accessible from the Portal navigation under **Lineage** (route **/x/lineage**).

<Steps>

### Open the Lineage page

Click **Lineage** in the navigation (the plugin adds it at **/x/lineage**). The full DAG for all pipelines loads.
...
Python pipelines use `ref()` within their SQL queries (executed through DuckDB). The lineage plugin scans the Python source for string patterns matching `ref('...')` to extract dependencies. This works reliably for inline SQL strings.

A Python pipeline is a flat script: the runner injects a `duckdb_conn` connection (plus `ref`, `landing_zone`, `pa`, etc.) and your script sets a `result` global. There is no `run()` function, no `return`, and no `conn` parameter.

```python filename="pipeline.py"
# config.yaml: merge_strategy: full_refresh
...
Lineage is computed **on-demand** by the plugin when you open the Lineage page or request it via the plugin API. The process is:
...
The plugin mounts its API under the plugin route prefix `/api/v1/x/lineage`.

```bash filename="Terminal"
# Get the full lineage graph
curl http://localhost:8080/api/v1/x/lineage/graph

# Scope the graph to a namespace
curl "http://localhost:8080/api/v1/x/lineage/graph?namespace=default"
```
