...
| **Execution timeout** | 60 seconds | Long-running queries are automatically cancelled (`QUERY_TIMEOUT_SECS`) |
| **Default result limit** | 1000 rows | Applied when the request does not specify a `limit` |
...
Response. Each column is an object with `name` and `type` (there is no separate `types` array), and the count is reported as `total_rows`:

```json filename="Response"
{
  "columns": [{ "name": "count_star()", "type": "BIGINT" }],
  "rows": [{ "count_star()": 42150 }],
  "total_rows": 1,
  "duration_ms": 23
}
```
