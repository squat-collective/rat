...
|---|---|---|
| `landing_zone_upload` | A file is uploaded to a specific landing zone | `namespace`, `zone_name` |
| `cron` | A cron schedule is due | `cron_expr` (5-field) |
| `pipeline_success` | Another pipeline completes successfully | `namespace`, `layer`, `pipeline` |
| `webhook` | An HTTP request hits the shared webhook endpoint with a valid token | *(auto-generated token)* |
| `file_pattern` | An uploaded filename matches a glob pattern | `namespace`, `zone_name`, `pattern` |
| `cron_dependency` | Cron is due **and** all dependency pipelines have new data | `cron_expr`, `dependencies[]` (list of `"ns.layer.pipeline"` strings) |
...
Creates a token that fires the pipeline when the shared webhook endpoint is called with it. RAT auto-generates the token when you create this trigger type — the token is a bare hex string (no prefix).
...
After creation, the API returns the webhook URL and the one-time token:

```json filename="Webhook Response"
{
  "id": "trigger-uuid",
  "webhook_url": "http://localhost:8080/api/v1/webhooks",
  "webhook_token": "9f2c1ab7d4e8..."
}
```

Call the shared webhook endpoint, passing the token in the `X-Webhook-Token` header. The endpoint looks up the trigger by token, so the trigger ID is **not** part of the URL:

```bash filename="Terminal"
curl -X POST http://localhost:8080/api/v1/webhooks \
  -H "X-Webhook-Token: 9f2c1ab7d4e8..."
```
...
```json filename="Trigger Config"
{
  ...
  "config": {
    ...
    "dependencies": [
      ...
      "default.silver.clean_customers"
    ]
  },
  "enabled": true
}
...
Each dependency is a single string in `"namespace.layer.pipeline"` format — not an object.
...
Triggers are nested under their pipeline, so updates use the pipeline-scoped path with `PUT`:

```bash filename="Terminal"
curl -X PUT http://localhost:8080/api/v1/pipelines/{ns}/{layer}/{name}/triggers/{trigger-id} \
  -H "Content-Type: application/json" \
  -d '{
    "enabled": false,
...
```bash filename="Terminal"
curl -X DELETE http://localhost:8080/api/v1/pipelines/{ns}/{layer}/{name}/triggers/{trigger-id}
```
