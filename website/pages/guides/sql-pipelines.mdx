...
| `{{ is_incremental() }}` | Boolean | `true` whenever the merge strategy is `incremental` |
...
`is_incremental()` returns `true` on every run when the strategy is `incremental`. On the **first run**, `watermark_value` is empty, so the `WHERE col > watermark` filter matches everything and RAT loads all data. On **subsequent runs**, `watermark_value` holds the previous `MAX(watermark_column)`, so only rows newer than the watermark are processed.
...
<Callout type="warning">
**Common mistake**: Forgetting `{% if is_incremental() %}` in an incremental pipeline. Without the watermark filter, every run reprocesses all data — defeating the purpose of incremental processing. On the first run, `watermark_value` is empty, so the filter selects everything automatically.
</Callout>
