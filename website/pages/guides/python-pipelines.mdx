...
| `is_incremental` | `bool` | `True` whenever the merge strategy is `incremental`. (The first run differs by an empty `watermark_value`, not by this flag.) |
| `config` | `PipelineConfig` or `None` | The parsed pipeline config as a **frozen `PipelineConfig` object** (read fields as attributes, e.g. `config.merge_strategy`). It is `None` when the pipeline has no config block — it is **not** a dict. |
...
You can read the pipeline's configuration through the `config` global. It is a frozen
`PipelineConfig` object (or `None` when no config block is present), so access its fields
as **attributes** — not with `config.get(...)`. The available fields are the documented
config keys: `merge_strategy`, `unique_key`, `watermark_column`, `materialized`,
`partition_by`, `max_retries`, `retry_delay_seconds`, etc. Arbitrary custom keys are
**not** supported — unknown keys in `config.yaml` are ignored with a warning.
...
```python filename="pipeline.py"
# `config` is a frozen PipelineConfig object (or None) — read attributes, not dict keys.
if config is not None and config.merge_strategy == "incremental":
    log.info(f"Incremental on key {config.unique_key}, watermark {config.watermark_column}")
...
| `getattr()` / `setattr()` / `delattr()` | Attribute access (can reach `__subclasses__`) |
| `type()` / `isinstance()` / `hasattr()` | Type introspection chains |
| `breakpoint()` | Debugger access |
| `input()` / `help()` | Interactive (would hang) |

<Callout type="info">
`__import__` is **not** removed — it is replaced with a *restricted* importer. Normal `import` statements work, but importing a blocked module raises `ImportError`. (PyArrow's C++ code calls `__import__` internally, so removing it entirely would crash the runner.)
</Callout>

### Blocked Imports

These modules cannot be imported (the importer raises `ImportError`):
...
| `http` / `ftplib` / `smtplib` | Network clients |
| `pathlib` / `tempfile` / `glob` | Filesystem paths |
| `importlib` | Dynamic imports |
| `ctypes` | C library access |
| `multiprocessing` / `threading` / `concurrent` | Process/thread spawning |
| `builtins` / `inspect` / `gc` / `types` / `abc` | Introspection / sandbox escape |
| `pickle` / `marshal` / `shelve` | (De)serialization escape vectors |

<Callout type="info">
Network libraries like `requests` and `urllib` are **not** on the blocklist — they simply are not installed in the pipeline environment. If you need network access, use DuckDB's HTTP extension via `duckdb_conn`.
</Callout>
