...
Quality tests run automatically when a **published** pipeline executes. (Unpublished pipelines can't run at all, so there's nothing to test until you publish.) RAT runs the `.sql` files captured in the pipeline's pinned published version — each test is read from its published snapshot, not from a live listing of the `tests/quality/` directory.
