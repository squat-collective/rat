...
When multiple rows in the pipeline output share the same `unique_key`, RAT keeps only **the last row**. It tags each incoming row with its insertion order and applies `ROW_NUMBER() OVER (PARTITION BY unique_key ORDER BY _rowid DESC)`, so the row that appears last in the result set (highest insertion order) wins.
...
- **Recommended annotations**: `partition_column` — without it, `snapshot` falls back to `full_refresh` (no error)
...
-- RIGHT: give snapshot a partition column
-- @merge_strategy: snapshot
-- @partition_column: region
```

Without `partition_column`, RAT has no partitions to overwrite, so `snapshot` quietly falls back to `full_refresh` (it does not fail) — usually not what you intended.
