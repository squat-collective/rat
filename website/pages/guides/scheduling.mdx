...
The request identifies the pipeline by `namespace` / `layer` / `pipeline` (plus a `cron` expression) — not by an internal `pipeline_id`.
...
<Callout type="info">
In a single-user, single-replica deployment, leader election happens automatically — there is only one instance to claim the lock. This is only relevant if you run multiple `ratd` replicas behind a load balancer.
</Callout>
