...
`is_incremental()` is `true` whenever the merge strategy is `incremental` — on **every** run, including the first. What actually differs between runs is the `watermark_value`:

### First Run (No Watermark Yet)

- `is_incremental()` returns `true` (the strategy is `incremental`)
- The `{% if is_incremental() %}` block **is included**
- But `watermark_value` is empty/`None` — there is no existing data to read a `MAX()` from
- With no watermark to compare against, the `WHERE col > watermark` filter selects **all** source rows (see the [NULL Watermarks](#null-watermarks) pattern below)
- RAT creates the target Iceberg table and writes all results
- The effect is a `full_refresh` for the first run

### Subsequent Runs (Watermark Set)

- `is_incremental()` still returns `true`
...
<Mermaid chart={`
flowchart TD
    A[Pipeline Run] --> B{watermark_value\nset?}
    B -->|No — First Run| C[is_incremental = true\nwatermark_value = None]
    C --> D[Filter selects ALL rows]
...
SELECT * FROM (
    ...
    ORDER BY _rowid DESC  -- last row by insertion order wins
) = 1
```

RAT tags each incoming row with its insertion order (`_rowid`) and keeps the row with the **highest** `_rowid` per key — i.e. the **last row** in the result set wins. All other duplicates are discarded before the merge.
...
{% if is_incremental() %}
-- This block is included on every incremental run; on the first run
-- watermark_value is empty, so the filter selects everything.
WHERE last_active_at > '{{ watermark_value }}'
{% endif %}
```

**First run** — `watermark_value` is empty, so the filter matches all rows. The SQL executed is effectively:

```sql
SELECT user_id, email, plan, last_active_at
FROM iceberg_scan('...')
-- watermark_value empty/NULL — all rows pass the filter
```
...
**Run 1** (no watermark yet):
1. `is_incremental()` = `true`, but `watermark_value` is empty -> filter matches everything
