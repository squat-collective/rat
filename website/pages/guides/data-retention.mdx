...
Upload file to landing zone
    │
    ▼
File at: {ns}/landing/{zone}/{filename}
    │
    ▼ (pipeline runs with archive_landing_zones: true)
File moved to: {ns}/landing/{zone}/_processed/{run_id}/{filename}
    │
    ▼ (reaper runs, zone has auto_purge: true)
File deleted after processed_max_age_days
```
