...
curl -X POST http://localhost:8080/api/v1/landing-zones \
  ...
  -d '{"namespace": "ecommerce", "name": "orders", "description": "Order CSV drops"}'
```
...
```bash filename="Terminal"
# Upload a single file (multipart form field: file)
curl -X POST http://localhost:8080/api/v1/landing-zones/ecommerce/orders/files \
...
Triggers are **not** defined in `config.yaml`. Create a `landing_zone_upload` trigger on the
pipeline via the triggers API (or the pipeline's **Settings → Triggers** panel in the portal):

```bash filename="Terminal"
curl -X POST http://localhost:8080/api/v1/pipelines/ecommerce/bronze/raw_orders/triggers \
  -H "Content-Type: application/json" \
  -d '{
    "type": "landing_zone_upload",
    "config": { "namespace": "ecommerce", "zone_name": "orders" }
  }'
```

Now whenever files are uploaded to the `orders` landing zone, this pipeline runs automatically.
See the [Pipeline Triggers](/guides/pipeline-triggers) guide for all trigger types.
...
Keep `config.yaml` for merge/archive settings only — triggers are created separately via the
triggers API (or the portal's **Settings → Triggers** panel):
...
```bash filename="Terminal"
# Create the landing_zone_upload trigger on the pipeline
curl -X POST http://localhost:8080/api/v1/pipelines/ecommerce/bronze/raw_orders/triggers \
  -H "Content-Type: application/json" \
  -d '{
    "type": "landing_zone_upload",
