...
| **Yes** | Snapshot strategy tables (a `partition_column` is recommended for `snapshot` — without one it falls back to `full_refresh`) |
...
The `snapshot` strategy works best with a `partition_column`. When one is set, the pipeline replaces only the partition matching the value produced by the query. If you omit `partition_column`, `snapshot` quietly falls back to `full_refresh` (no error):
...
5. **Combine with `partition_column` for snapshots** — the snapshot strategy gives you idempotent partition-level overwriting only when one is set; without it, `snapshot` falls back to `full_refresh`
