...
| 42 | Jane Smith | 456 Oak Ave | 2026-02-15 | _NULL_ |

The `valid_from` and `valid_to` columns define the time range during which each version was the "current" record. The current (open) version always has `valid_to = NULL` — that NULL is how RAT marks "still active". There is **no** `9999-12-31` far-future sentinel.
...
3. For **new records**: inserts them with `valid_from = current_timestamp`, `valid_to = NULL`
4. For **changed records**: closes the current version (sets `valid_to = current_timestamp`) and inserts a new version with `valid_from = current_timestamp`, `valid_to = NULL`
...
|---|---|---|---|---|---|
| 42 | Jane Smith | 123 Elm St | silver | 2026-02-15 10:00:00 | _NULL_ |
| 17 | Bob Jones | 789 Pine Rd | gold | 2026-02-15 10:00:00 | _NULL_ |
...
| 42 | Jane Smith | **456 Oak Ave** | silver | **2026-02-16 10:00:00** | _NULL_ |
| 17 | Bob Jones | 789 Pine Rd | gold | 2026-02-15 10:00:00 | _NULL_ |
...
| 42 | Jane Smith | 456 Oak Ave | **gold** | **2026-02-17 10:00:00** | _NULL_ |
| 17 | Bob Jones | 789 Pine Rd | gold | 2026-02-15 10:00:00 | _NULL_ |
...
A `NULL valid_to` always identifies the current (active) version.

<Callout type="warning">
Always test the open row with `valid_to IS NULL` (not `valid_to = '9999-12-31'`). RAT leaves the current version's `valid_to` as NULL — any equality or `<` comparison against it evaluates to NULL (not true), which would silently drop every current row from your results.
</Callout>
...
WHERE customer_id = 42
  ...
  AND (valid_to > TIMESTAMP '2026-02-16 12:00:00' OR valid_to IS NULL)
```

This returns the version that was active at noon on February 16th: `456 Oak Ave, tier=silver`. The `OR valid_to IS NULL` clause is essential so the current (open) version is included when your point-in-time falls within it.
...
This is the primary value of SCD2 — joining facts to the correct dimension version at transaction time. The `OR c.valid_to IS NULL` term matches orders that fall within the customer's current (open) version; without it, recent orders would silently fail to join.
...
SELECT
    ...
    valid_from,
    valid_to,
    CASE WHEN valid_to IS NULL THEN 'current' ELSE 'historical' END AS version_status
FROM "default"."silver"."customer_history"
WHERE customer_id = 42
ORDER BY valid_from
```
...
- **Filter on `valid_to`** — most queries only need current records. Add `WHERE valid_to IS NULL` when you don't need history
